...
if (-not $isPr -and $env:SkipAssemblySigning -ne "true") {
  ...
  while ($sw.elapsed -lt $timeout -and $polling) {
    $blob = Get-AzureStorageBlob "$env:APPVEYOR_BUILD_VERSION.zip" "azure-functions-esrp-nuget-signed" -Context $ctx -ErrorAction Ignore
    if (-not $blob) {
  ...
  Get-AzureStorageBlobContent "$env:APPVEYOR_BUILD_VERSION.zip" "azure-functions-esrp-nuget-signed" -Destination "$directoryPath/../buildoutput/signed.zip" -Context $ctx
